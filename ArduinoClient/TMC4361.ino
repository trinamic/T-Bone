...
void initialzeTMC4361() {
  ...
  for (char i=0; i<nr_of_motors;i++) {
    write43x(i, TMC4361_GENERAL_CONFIG_REGISTER, 0); //we use direct values
    write43x(i, TMC4361_RAMP_MODE_REGISTER,_BV(2) | 2); //we want to go to positions in nice S-Ramps)
    write43x(i, TMC4361_SH_RAMP_MODE_REGISTER,_BV(2) | 2); //we want to go to positions in nice S-Ramps)
    write43x(i,TMC4361_CLK_FREQ_REGISTER,CLOCK_FREQUENCY);
    write43x(i,TMC4361_START_DELAY_REGISTER, 256); //NEEDED so THAT THE SQUIRREL CAN RECOMPUTE EVERYTHING!
...
const __FlashStringHelper* setStepsPerRevolution(unsigned char motor_nr, unsigned int steps) {
  ...
  motorconfig |= steps<<4;
  write43x(motor_nr,TMC4361_STEP_CONF_REGISTER,motorconfig);
...
unsigned long homming_jerk)
{
  ...
  write43x(motor_nr, TMC4361_START_CONFIG_REGISTER, 0
    | _BV(10)//immediate start        
  //since we just start 
  );   
  write43x(motor_nr, TMC4361_A_MAX_REGISTER,homming_accel); //set maximum acceleration
  write43x(motor_nr, TMC4361_D_MAX_REGISTER,homming_accel); //set maximum deceleration
  write43x(motor_nr,TMC4361_BOW_1_REGISTER,homming_jerk);
  write43x(motor_nr,TMC4361_BOW_2_REGISTER,homming_jerk);
  write43x(motor_nr,TMC4361_BOW_3_REGISTER,homming_jerk);
  write43x(motor_nr,TMC4361_BOW_4_REGISTER,homming_jerk);
  ...
  while (homed!=0xff) { //we will never have 255 homing phases - but whith this we not have to think about it 
    if (homed==0 || homed==1) {
      ...
      unsigned long status = read43x(motor_nr, TMC4361_STATUS_REGISTER,0);
      unsigned long events = read43x(motor_nr, TMC4361_EVENTS_REGISTER,0);
      if (!(status & (_BV(7) | _BV(9)))) {
...
#endif
          target -= 1000l;
          write43x(motor_nr,TMC4361_V_MAX_REGISTER, FIXED_23_8_MAKE(homing_speed));
          write43x(motor_nr, TMC4361_X_TARGET_REGISTER,target);
        }
      } 
      else {
        long go_back_to;
        if (homed==0) {
          long actual = read43x(motor_nr, TMC4361_X_ACTUAL_REGISTER,0);
          ...
          long actual = read43x(motor_nr, TMC4361_X_LATCH_REGISTER,0);
          go_back_to = actual;
#ifdef DEBUG_HOMING
        ...
        write43x(motor_nr,TMC4361_V_MAX_REGISTER, FIXED_23_8_MAKE(homing_fast_speed));
        write43x(motor_nr, TMC4361_X_TARGET_REGISTER,go_back_to);
        delay(10ul);
        status = read43x(motor_nr, TMC4361_STATUS_REGISTER,0);
        while (!(status & _BV(0))) {
          status = read43x(motor_nr, TMC4361_STATUS_REGISTER,0);
        }
        if (homed==0) {
          ...
          write43x(motor_nr, TMC4361_X_ACTUAL_REGISTER,0);
          homed=0xff;
        }     
      } 
    } 
...
inline long getMotorPosition(unsigned char motor_nr) {
  return read43x(motor_nr, TMC4361_X_TARGET_REGISTER ,0);
...
void moveMotor(unsigned char motor_nr, long target_pos, double vMax, double aMax, long jerk, boolean configure_shadow, boolean isWaypoint) {
  ...
  if (!configure_shadow) {
    write43x(motor_nr, TMC4361_V_MAX_REGISTER,FIXED_23_8_MAKE(vMax)); //set the velocity 
    write43x(motor_nr, TMC4361_A_MAX_REGISTER,fixed_a_max); //set maximum acceleration
    write43x(motor_nr, TMC4361_D_MAX_REGISTER,fixed_a_max); //set maximum deceleration
    write43x(motor_nr, TMC4361_BOW_1_REGISTER,jerk);
    write43x(motor_nr, TMC4361_BOW_2_REGISTER,jerk);
    write43x(motor_nr, TMC4361_BOW_3_REGISTER,jerk);
    write43x(motor_nr, TMC4361_BOW_4_REGISTER,jerk);
    //TODO pos comp is not shaddowwed
    next_pos_comp[motor_nr] = target_pos;
    write43x(motor_nr, TMC4361_POS_COMP_REGISTER,target_pos);

  } 
  else {
    write43x(motor_nr, TMC4361_SH_V_MAX_REGISTER,FIXED_23_8_MAKE(vMax)); //set the velocity 
    write43x(motor_nr, TMC4361_SH_A_MAX_REGISTER,fixed_a_max); //set maximum acceleration
    write43x(motor_nr, TMC4361_SH_D_MAX_REGISTER,fixed_a_max); //set maximum deceleration
    write43x(motor_nr, TMC4361_SH_BOW_1_REGISTER,jerk);
    write43x(motor_nr, TMC4361_SH_BOW_2_REGISTER,jerk);
    write43x(motor_nr, TMC4361_SH_BOW_3_REGISTER,jerk);
    write43x(motor_nr, TMC4361_SH_BOW_4_REGISTER,jerk);

    //TODO pos comp is not shaddowwed
    next_pos_comp[motor_nr] = target_pos;

  }
  write43x(motor_nr, TMC4361_X_TARGET_REGISTER,aim_target);
...
inline void signal_start() {
  ...
  for (char i=0; i< nr_of_motors; i++) {
    ...
    read43x(i, TMC4361_EVENTS_REGISTER,0);
    write43x(i, TMC4361_POS_COMP_REGISTER,next_pos_comp[i]);
    if (target_motor_status & _BV(i)) {
      unsigned long motor_pos = read43x(i, TMC4361_X_ACTUAL_REGISTER,0);
...
inline unsigned long getClearedEndStopConfig(unsigned char motor_nr, boolean left) {
  unsigned long endstop_config = read43x(motor_nr, TMC4361_REFERENCE_CONFIG_REGISTER, 0);
  //clear everything
